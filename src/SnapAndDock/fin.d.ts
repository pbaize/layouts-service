...
/**
 * JavaScript API
 * The JavaScript API allows you to create an HTML/JavaScript application that has access to the native windowing environment, can communicate with other
 * applications and has access to sandboxed system-level features.
 *
 * API Ready
 * When using the OpenFin API, it is important to ensure that it has been fully loaded before making any API calls. To verify that the API is in fact ready, be
 * sure to make any API calls either from within the fin.desktop.main() method or explicitly after it has returned. This avoids the situation of trying to
 * access methods that are not yet fully injected.
 *
 * Overview
 * When running within the OpenFin Runtime your web applications have access to the "fin" namespace and all the modules within the API without the need to
 * include additional source files. You can treat the "fin" namespace as you would the "window", "navigator" or "document" objects.
 */
declare namespace fin {
    ...
    interface OpenFinApplicationStatic {
        ...
        /**
         * Starts a new application. First parameter is the URL to an application manifest (an app.json file).
         */
        createFromManifest(manifestUrl: string, callback: (app: OpenFinApplication) => void, errorCallback: (reason: any) => void): void;
    }
    ...
    interface OpenFinApplication {
        ...
        /**
         * Retrieves an array of active window groups for all of the application's windows. Each group is represented as an array of wrapped
         * fin.desktop.Windows.
         */
        ...
        /**
         * Sets new shortcut configuration for current application. Application has to be launched with a manifest and has to have shortcut configuration (icon
         * url, name, etc.) in its manifest to be able to change shortcut states.
         */
        setShortcuts(config: ShortCutConfig, callback?: () => void, errorCallback?: (reason: string) => void): void;
        /**
         * Adds a customizable icon in the system tray and notifies the application when clicked.
         */
        setTrayIcon(iconUrl: string, listener: (clickInfo: TrayIconClickedEvent) => void, callback?: () => void, errorCallback?: (reason: string) => void):
            void;
        /**
        ...
        /**
         * Waits for a hanging application. This method can be called in response to an application "not-responding" to allow the application to continue and to
         * generate another "not-responding" message after a certain period of time.
         */
    ...
    interface WindowOptions {
         ...
         */
        accelerator?: {devtools?: boolean, zoom?: boolean};
        /**
         ...
         */
        cornerRounding?: {width?: number; height?: number;};
        /**
         * A field that the user can attach serializable data to to be ferried around with the window options. Default: ''.
         */
        customData?: any;
        /**
         * Specifies that the window will be positioned in the center of the primary monitor when loaded for the first time on a machine. When the window
         * corresponding to that id is loaded again, the position from before the window was closed is used. This option overrides defaultLeft and defaultTop.
         * Default: false.
         */
        defaultCentered?: boolean;
        /**
         * The default height of the window. Specifies the height of the window when loaded for the first time on a machine. When the window corresponding to
         * that id is loaded again, the height is taken to be the last height of the window before it was closed. Default: 500.
         */
        defaultHeight?: number;
        /**
         * The default left position of the window. Specifies the position of the left of the window when loaded for the first time on a machine. When the
         * window corresponding to that id is loaded again, the value of left is taken to be the last value before the window was closed. Default: 100.
         */
        defaultWidth?: number;
        /**
         * The default top position of the window. Specifies the position of the top of the window when loaded for the first time on a machine. When the window
         * corresponding to that id is loaded again, the value of top is taken to be the last value before the window was closed. Default: 100.
         */
        defaultTop?: number;
        /**
         * The default width of the window. Specifies the width of the window when loaded for the first time on a machine. When the window corresponding to that
         * id is loaded again, the width is taken to be the last width of the window before it was closed. Default: 800.
         */
        defaultLeft?: number;
        /**
        ...
        url?: string;
        /**
         * When set to false, the window will render before the "load" event is fired on the content's window. Caution, when false you will see an initial empty
    ...
    interface OpenFinExternalApplication {
         ...
         */
        addEventListener(
            type: OpenFinExternalApplicationEventType, listener: () => void, callback?: () => void,
    ...
    interface OpenFinInterApplicationBus {
        ...
        /**
         * Subscribes to messages from the specified application on the specified topic. If the subscription is for a uuid, [name], topic combination that has
         * already been published to upon subscription you will receive the last 20 missed messages in the order they were published.
         */
        subscribe(
            senderUuid: string, name: string, topic: string, listener: (message: any, uuid: string, name: string) => void, callback?: () => void,
            errorCallback?: (reason: string) => void): void;
        subscribe(
            senderUuid: string, topic: string, listener: (message: any, uuid: string, name: string) => void, callback?: () => void,
            errorCallback?: (reason: string) => void): void;
        /**
    ...
    interface OpenFinNotificationStatic {
        ...
        /**
         * Gets an instance of the current notification. For use within a notification window to close the window or send a message back to its parent
         * application.
         */
    ...
    /**
     * Notification
     * Notification represents a window on OpenFin Runtime which is shown briefly to the user on the bottom-right corner of the primary monitor. A notification
     * is typically used to alert the user of some important event which requires his or her attention. Notifications are a child or your application that are
    ...
    interface OpenFinNotification {
        ...
        /**
         * Sends a message from the notification to the application that created the notification. The message is handled by the notification's onMessage
         * callback.
         */
    ...
    interface NotificationOptions {
        ...
        /**
         * Invoked when the notification is closed via .close() method on the created notification instance or the by the notification itself via
         * fin.desktop.Notification.getCurrent().close(). NOTE: this is not invoked when the notification is dismissed via a swipe. For the swipe dismissal
         * callback see onDismiss
         */
        ...
        /**
         * The onMessage function will respond to messages sent from notification.sendMessageToApplication.The function is passed the message, which can be of
         * any primitive or composite-primitive type.
         */
    ...
    interface OpenFinServiceClient {
        dispatch: (action: string, payload?: any) => Promise<any>;
        register: (action: string, listener: (payload: any, identity: OpenFinIdentity) => any) => void;
    }

    interface OpenFinServiceProvider {
        publish: (action: string, payload: any) => void;
        dispatch: (to: OpenFinIdentity, action: string, payload: any) => Promise<any>;
        register: (action: string, listener: (payload: any, identity: OpenFinIdentity) => any) => void;
    }
    ...
    /**
     * System
     * An object representing the core of OpenFin Runtime. Allows the developer to perform system-level actions, such as accessing logs, viewing processes,
    ...
    interface OpenFinSystem {
         ...
         */
        downloadAsset(
            assetObj: AppAssetInfo, progressListener?: (progress: {downloadedBytes: number, totalBytes: number}) => void,
        ...
        /**
         * Retrieves an array of all of the runtime processes that are currently running. Each element in the array is an object containing the uuid and the
         * name of the application to which the process belongs.
         */
        ...
        terminateExternalProcess(
            processUuid: string, timeout: number, killTree: boolean, callback?: (info: {result: 'clean'|'terminated'|'failed'}) => void,
            errorCallback?: (reason: string) => void): void;
        terminateExternalProcess(
            processUuid: string, timeout: number, callback?: (info: {result: 'clean'|'terminated'|'failed'}) => void,
            errorCallback?: (reason: string) => void): void;
        /**
    ...
    interface OpenFinWindowStatic {
         ...
         *
         * A basic window that wraps a native HTML window. Provides more fine-grained control over the window state such as the ability to minimize, maximize,
         * restore, etc. By default a window does not show upon instantiation; instead the window's show() method must be invoked manually. The new window
         * appears in the same process as the parent window.
     ...
     * Window
     * A basic window that wraps a native HTML window. Provides more fine-grained control over the window state such as the ability to minimize, maximize,
     * restore, etc. By default a window does not show upon instantiation; instead the window's show() method must be invoked manually. The new window appears
     * in the same process as the parent window.
     */
    interface OpenFinWindow {
        ...
        name: string;
        /**
         * Returns the native JavaScript "window" object for the window. This method can only be used by the parent application or the window itself, otherwise
         * it will return undefined. The same Single-Origin-Policy (SOP) rules apply for child windows created by window.open(url) in that the contents of the
         * window object are only accessible if the URL has the same origin as the invoking window. See example below. Also, will not work with
         * fin.desktop.Window objects created with fin.desktop.Window.wrap().
        ...
        /**
         * Retrieves an array containing wrapped fin.desktop.Windows that are grouped with this window. If a window is not in a group an empty array is
         * returned. Please note that calling window is included in the result array.
         */
        ...
        /**
         * Shows the window if it is hidden at the specified location. If the toggle parameter is set to true, the window will alternate between showing and
         * hiding.
         */
    ...
    interface MonitorInfo {
        ...
        reason: string;
        taskbar: {edge: 'left'|'right'|'top'|'bottom', rect: MontiorCoordinates};
        topic: 'system';
        type: 'monitor-info-changed';
